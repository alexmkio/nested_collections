...
p denver_biscuit_co[:hours].first[:open].find {|day_obj| day_obj[:day] === 1} [:start]

# Return the closing hour on Saturday (day 5) ("1500")
p denver_biscuit_co[:hours].first[:open].find {|day_obj| day_obj[:day] === 5} [:end]
...
denver_biscuit_co[:hours].first[:open].each_with_index {|day, i|
    compiled_object[days[day[:day]]] = {
        start: day[:start],
        end: day[:end]
    }
}
